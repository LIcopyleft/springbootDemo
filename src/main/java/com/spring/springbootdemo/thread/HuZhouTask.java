...
public class HuZhouTask implements Runnable {

    private int beginIndex;
    private ConfigParam config;
    static DataContentMapper mapper = SpringContextHolder.getBean("dataContentMapper");
    private static final Logger logger = LoggerFactory.getLogger(HuZhouTask.class);


    public HuZhouTask(int beginIndex, ConfigParam config) {
   ...
   //     data.setCategory(memu);
        ...
        //信息类型  政府采购>集中采购>集中采购招标公告

        String category = data.getCategory();
        String[] split = category.split("\\>");
        data.setStageShow(split[2]);
        // 业务类型
