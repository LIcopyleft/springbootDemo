...
public class ShanXi5Task implements Runnable {

    private int beginIndex;
    private ConfigParam config;
    static DataContentMapper mapper = SpringContextHolder.getBean("dataContentMapper");
    private static final Logger logger = LoggerFactory.getLogger(ShanXi5Task.class);


    public ShanXi5Task(int beginIndex, ConfigParam config) {
    ...
    //    data.setStageShow(data.getCategory().split("\\>")[2]);
        String stageShow = data.getStageShow();
        if ("jiaoyi".equals(stageShow)){
            data.setStageShow("交易信息");
        }else if("gonggao".equals(stageShow)){
            data.setStageShow("交易公告");
        }
