...
public class JiangXiTask implements Runnable {

    private int beginIndex;
    private ConfigParam config;
    static DataContentMapper mapper = SpringContextHolder.getBean("dataContentMapper");
    private static final Logger logger = LoggerFactory.getLogger(JiangXiTask.class);


    public JiangXiTask(int beginIndex, ConfigParam config) {
    ...
    public void run() {
        try {
            ...
            //    List<DataContentWithBLOBs> dataContent = mapper.selectAll(beginIndex, config.getQuerySize(), config.getCleanTableName());
          /*  List<DataContentWithBLOBs> dataContent = null;
            List<Integer> urlIds = mapper.selectUrlIds("spider_10_ggzy_anhui_url", beginIndex, config.getQuerySize());
            if (urlIds != null && urlIds.size() > 0) {
              *//*  StringBuffer sb = new StringBuffer();
                ...
                dataContent = mapper.anHui(urlIds);
            }*/
            //   mapper.selectInList();
    ...
    public static GovData clean_zbgg(GovData data) throws InvocationTargetException, IllegalAccessException {
        if (data.getUrlId() == 27964 || data.getUrlId() == 197) {
            data.getUrlId();
        }
   ...
   //     data.setCategory(memu);
        ...
        //信息类型
        data.setStageShow(data.getCategory().replaceAll("交易公告>政府采购>", ""));
        // 业务类型
