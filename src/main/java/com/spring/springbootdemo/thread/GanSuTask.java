...
public class GanSuTask implements Runnable {
    ...
    public GanSuTask(int beginIndex, ConfigParam config) {
    ...
    public static GovData clean_zbgg(GovData data) throws InvocationTargetException, IllegalAccessException {
       ...
       /* if (pubTime == null) {
        ...
        Elements sitemap = parse.getElementsByClass("sTradingInformationNav");
        Elements info = parse.getElementsByClass("sProcessHeader");
        String noticeTime = RegExpUtil.regGet(content, "项目进场日期[\\S\\s]{0,50}");
        data.setNoticeTime(noticeTime);
        String stageShow = null;
        if (info != null && info.size() > 0) {
            String text = info.text();
            if (text != null && text.contains("-")) {
                text = RegExpUtil.regGet(text, "\\S{0,}-\\S{0,}");
                stageShow = text.split("-")[1];
            }
        }
        if (data.getCategory().equals("政府采购限额以下")) {

            Elements head = parse.getElementsByClass("containerBorrow");
            stageShow = head.select("active").text();
